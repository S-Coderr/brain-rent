...
class VoiceRecorder {
  constructor(opts = {}) {
    ...
    if (this.mainBtn) this.mainBtn.addEventListener('click', () => this.toggle());
    if (this.trashBtn) this.trashBtn.addEventListener('click', () => this.delete());
    if (this.playBtn) this.playBtn.addEventListener('click', () => this.play());
  }

  async toggle() {
    if (!this.recording) await this.start(); else this.stop();
  }
  ...
  async start() {
    try {
      ...
      if (this.mainBtn) { this.mainBtn.innerHTML = '⏹'; this.mainBtn.classList.add('stop'); }
      ...
      this.waveInt = setInterval(() => {
        document.querySelectorAll(`#${this.waveEl?.id} .br-wave-bar`).forEach(b => {
  ...
  stop() {
    ...
    if (this.mainBtn) { this.mainBtn.innerHTML = '🎙️'; this.mainBtn.classList.remove('stop'); }
    if (this.statusEl) this.statusEl.textContent = '✓ Recording saved';
    document.querySelectorAll(`#${this.waveEl?.id} .br-wave-bar`).forEach(b => { b.style.height = '4px'; b.classList.remove('active'); });
  }
...
const BrainRent = {
  ...
  toast(msg, type = 'success') {
    ...
    if (!el) {
      ...
      el.innerHTML = '<span class="br-toast-icon"></span><span class="br-toast-msg"></span>';
      document.body.appendChild(el);
    }
    el.className = `br-toast ${type}`;
    el.querySelector('.br-toast-icon').textContent = type === 'success' ? '✓' : '✗';
  ...
  async loadNotifications() {
    ...
    try {
      ...
      if (!notifications.length) {
        listEl.innerHTML = '<div class="text-center text-muted py-4 small">No notifications yet</div>';
        return;
      }
...
function initSubmitWizard() {
  ...
  if (txt && cnt) txt.addEventListener('input', () => cnt.textContent = `${txt.value.length} / 3000 characters`);

  // Next button
  document.getElementById('btn-next')?.addEventListener('click', () => {
    const title = document.getElementById('problem-title')?.value.trim();
    if (!title) { BrainRent.toast('Please add a problem title', 'error'); return; }
    document.getElementById('review-title').textContent   = title;
    const body = document.getElementById('problem-text')?.value || '';
    document.getElementById('review-excerpt').textContent = body.length > 130 ? body.substring(0,130)+'…' : body || '(Voice recording only)';
  ...
  document.querySelectorAll('.br-urgency-opt').forEach(opt => {
    opt.addEventListener('click', () => {
      ...
      document.querySelectorAll('.sidebar-urgency').forEach(el => el.textContent = '$' + price);
      document.querySelectorAll('.sidebar-total').forEach(el => el.textContent  = '$' + total);
  ...
  fileInput?.addEventListener('change', () => {
    ...
    Array.from(fileInput.files).forEach(f => {
      ...
      item.style.cssText = 'display:flex;align-items:center;gap:8px;background:var(--br-dark3);border-radius:8px;padding:8px 12px;font-size:.8rem;margin-top:6px';
  ...
  document.getElementById('btn-pay')?.addEventListener('click', async () => {
    ...
    try {
      const res = await fetch(apiUrl('/api/submit_request.php'), { method:'POST', body: formData });
      const data = await res.json();
      if (!data.success) {
      ...
      const rz = new Razorpay({
        ...
        name:        'BrainRent',
        description: 'Thinking Session',
        handler: async function(response) {
          await BrainRent.post(apiUrl('/api/verify_payment.php'), response);
          step2.style.display = 'none';
          step3.style.display = 'block';
          setStepActive(3);
          BrainRent.toast('Payment successful! Request submitted.', 'success');
        },
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.querySelectorAll('[data-action]').forEach(btn => {
    btn.addEventListener('click', async function(e) {
      ...
      const res  = await fetch(apiUrl('/api/manage_request.php'), { method:'POST', body: fd });
      const data = await res.json();

      BrainRent.toast(data.success ? data.message : (data.error || 'Error'), data.success ? 'success' : 'error');
