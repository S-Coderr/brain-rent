...
$newReqs = $db->fetchAll("SELECT tr.*, u.full_name AS client_name FROM thinking_requests tr INNER JOIN users u ON tr.client_id = u.id WHERE tr.expert_id = ? AND tr.status = 'submitted' ORDER BY tr.urgency DESC, tr.created_at", [$userId]);
$activeReqs = $db->fetchAll("SELECT tr.*, u.full_name AS client_name FROM thinking_requests tr INNER JOIN users u ON tr.client_id = u.id WHERE tr.expert_id = ? AND tr.status IN ('accepted','thinking') ORDER BY tr.response_deadline", [$userId]);
...
<div style="padding-top:64px;display:flex;height:calc(100vh - 64px)">
  ...
  <div class="br-sidebar">
    <div class="mb-3 px-2">
      ...
      <?php if ($profile['is_verified']): ?><span class="br-badge br-badge-teal mt-1" style="font-size:.68rem">✓ Verified</span><?php endif; ?>
    </div>
    <a href="#" class="br-nav-item active" onclick="showSection('exp-overview',this)"><i class="bi bi-grid me-2"></i>Overview</a>
    ...
    <a href="#" class="br-nav-item" onclick="showSection('exp-respond',this)"><i class="bi bi-mic me-2"></i>Submit Response</a>
    <a href="#" class="br-nav-item" onclick="showSection('exp-wallet',this)"><i class="bi bi-wallet2 me-2"></i>Wallet & Earnings</a>
    <a href="#" class="br-nav-item" onclick="showSection('exp-settings',this)"><i class="bi bi-gear me-2"></i>Profile Settings</a>
    <div style="margin-top:auto;padding-top:16px;border-top:1px solid var(--br-border)">
      <a href="<?= APP_URL ?>/pages/dashboard-client.php" class="br-nav-item"><i class="bi bi-person me-2"></i>Client Mode</a>
    </div>
  </div>
  ...
  <div class="br-dash-main">
    ...
    <div id="section-exp-overview">
      <div class="mb-4">
        ...
        <p class="text-muted small"><?= count($newReqs) ?> new request(s) awaiting your review.</p>
      </div>
      ...
      <div class="br-wallet-card mb-4">
        ...
        <div class="text-muted small mt-1">$<?= number_format($wallet['pending_balance'], 2) ?> pending · $<?= number_format($wallet['total_earned'], 2) ?> total earned</div>
        <div class="d-flex gap-2 mt-3">
          <button class="btn br-btn-ghost btn-sm" onclick="BrainRent.toast('Withdrawal initiated!','success')">Withdraw Funds</button>
      ...
      <?php if ($newReqs): ?>
        <div class="br-table mb-4">
          <div class="d-flex justify-content-between align-items-center p-3" style="border-bottom:1px solid var(--br-border)">
            <h6 class="mb-0 fw-semibold">📥 New Requests</h6>
            <span class="br-badge br-badge-gold"><?= count($newReqs) ?> new</span>
          </div>
            ...
            <table class="table br-table mb-0">
              ...
              <tbody>
                <?php foreach ($newReqs as $req): ?>
                  <tr>
                    <td>
                      <div class="fw-medium small"><?= htmlspecialchars(substr($req['title'], 0, 55)) ?><?= strlen($req['title']) > 55 ? '…' : '' ?></div>
                    </td>
        ...
        <div class="mb-4">
          ...
          <select class="br-form-control form-control" id="respond-select" onchange="loadRequestDetail(this.value)">
            ...
            <?php foreach ($activeReqs as $r): ?>
              <option value="<?= $r['id'] ?>"><?= htmlspecialchars(substr($r['title'], 0, 70)) ?> · <?= htmlspecialchars($r['client_name']) ?></option>
            <?php endforeach; ?>
          </select>
        </div>
      <?php endif; ?>
          ...
          <div class="col-12 col-lg-8">
            <div class="br-profile-section mb-3">
              ...
              <div class="br-recorder">
                ...
                <div id="exp-rec-preview" style="display:none;margin-top:10px"><audio id="exp-rec-audio" controls class="w-100"></audio></div>
              </div>
            </div>
            <div class="br-profile-section mb-3">
              <div class="mb-3">
                <label class="br-form-label">📝 Written Analysis *</label>
                <textarea name="written_response" class="br-form-control form-control" rows="6" placeholder="Your structured analysis and reasoning…"></textarea>
              </div>
              <div class="mb-3">
                <label class="br-form-label">💡 Key Insights <span class="text-subtle">(one per line)</span></label>
                <textarea name="key_insights" class="br-form-control form-control" rows="4" placeholder="1. At your current scale, pgvector is underrated&#10;2. Your Postgres expertise is a genuine moat here&#10;3. Pinecone's real cost isn't what you think"></textarea>
              </div>
              <div class="mb-3">
                <label class="br-form-label">☐ Action Items for Client <span class="text-subtle">(one per line)</span></label>
                <textarea name="action_items" class="br-form-control form-control" rows="4" placeholder="1. Run pgvector HNSW benchmark with your actual data&#10;2. Get Pinecone enterprise pricing before deciding&#10;3. Read the 2024 ANN benchmarks comparison"></textarea>
              </div>
              <div class="mb-3">
                <label class="br-form-label">🔗 Resource Links <span class="text-subtle">(one per line)</span></label>
                <textarea name="resource_links" class="br-form-control form-control" rows="3" placeholder="https://pgvector.github.io&#10;https://ann-benchmarks.com"></textarea>
              </div>
              <div>
                <label class="br-form-label">⏱️ Time Spent (minutes)</label>
                <input type="number" name="thinking_minutes" class="br-form-control form-control" placeholder="e.g. 25" style="max-width:160px">
              </div>
            </div>
            <div class="d-flex justify-content-end gap-2">
              <button type="button" class="btn br-btn-ghost">Save Draft</button>
              <button type="submit" class="btn br-btn-gold px-4">Submit Response <i class="bi bi-arrow-right ms-1"></i></button>
            </div>
          </div>
            ...
            <div class="br-profile-section mb-3">
              ...
              <div class="d-flex flex-column gap-2 small">
                <?php foreach (['Voice recording included', 'At least 3 key insights', 'Clear, actionable items', 'Resource links added', 'Addressed client urgency'] as $item): ?>
                  <label class="d-flex align-items-center gap-2"><input type="checkbox" style="accent-color:var(--br-gold)"> <?= $item ?></label>
                <?php endforeach; ?>
              </div>
            </div>
    ...
    <div id="section-exp-wallet" style="display:none">
      ...
      <div class="br-wallet-card mb-4">
        <div class="text-subtle small mb-1" style="text-transform:uppercase;letter-spacing:1px">Available to Withdraw</div>
        <div class="br-wallet-balance">$<?= number_format($wallet['available_balance'], 2) ?></div>
        <div class="text-muted small mt-1">$<?= number_format($wallet['pending_balance'], 2) ?> pending · Releases after client confirmation</div>
        <div class="d-flex gap-2 mt-3">
          <button class="btn br-btn-gold btn-sm" onclick="BrainRent.toast('Withdrawal initiated!','success')">Withdraw to Bank</button>
          <button class="btn br-btn-ghost btn-sm" data-bs-toggle="modal" data-bs-target="#walletModal">Add Bank Account / UPI</button>
        </div>
      </div>
      ...
      <div class="br-profile-section mb-4">
        ...
        <div class="mt-3">
          <button class="btn br-btn-outline btn-sm" data-bs-toggle="modal" data-bs-target="#walletModal">Edit Payout Details</button>
        </div>
      </div>
      <div class="row g-3 mb-4">
        <div class="col-6 col-md-3">
            ...
            <tbody>
              <?php foreach ($recentEarnings as $e): ?>
                <tr>
                  ...
                  <td><span class="br-status <?= $e['status'] === 'released' ? 'br-status-completed' : 'br-status-thinking' ?>"><?= ucfirst($e['status']) ?></span></td>
        ...
        <div class="col-12 col-lg-8">
          <div class="br-profile-section">
            <form method="POST" action="<?= APP_URL ?>/api/update_profile.php">
              <div class="mb-3"><label class="br-form-label">Headline</label><input type="text" name="headline" class="br-form-control form-control" value="<?= htmlspecialchars($profile['headline'] ?? '') ?>"></div>
              <div class="mb-3"><label class="br-form-label">Bio</label><textarea name="bio" class="br-form-control form-control" rows="5"><?= htmlspecialchars($profile['bio'] ?? '') ?></textarea></div>
              <div class="mb-3"><label class="br-form-label">Expertise Areas <span class="text-subtle">(comma-separated)</span></label>
                <input type="text" name="expertise_areas" class="br-form-control form-control" value="<?= htmlspecialchars(implode(', ', json_decode($profile['expertise_areas'] ?? '[]', true) ?? [])) ?>">
              </div>
              <div class="row g-3 mb-3">
                <div class="col-6"><label class="br-form-label">Session Rate (USD)</label><input type="number" name="rate_per_session" class="br-form-control form-control" value="<?= $profile['rate_per_session'] ?>"></div>
                <div class="col-6"><label class="br-form-label">Max Response Hours</label><input type="number" name="max_response_hours" class="br-form-control form-control" value="<?= $profile['max_response_hours'] ?>"></div>
                <div class="col-6"><label class="br-form-label">Max Concurrent Requests</label><input type="number" name="max_active_requests" class="br-form-control form-control" value="<?= $profile['max_active_requests'] ?>"></div>
              ...
              <button type="button" class="btn br-btn-gold" onclick="BrainRent.toast('Profile updated!','success')">Save Changes</button>
            </form>
          </div>
        </div>
      </div>
        ...
        <div class="modal-body">
          ...
          <div class="row g-3">
            <div class="col-12 col-md-6">
              ...
              <input type="text" name="bank_account_name" class="br-form-control form-control" value="<?= htmlspecialchars($bankName) ?>" placeholder="e.g. Rahul Sharma">
            </div>
            <div class="col-12 col-md-6">
              <label class="br-form-label">Account Number</label>
              <input type="text" name="bank_account_number" class="br-form-control form-control" value="<?= htmlspecialchars($bankNumber) ?>" placeholder="e.g. 1234567890">
            </div>
            <div class="col-12 col-md-6">
              <label class="br-form-label">IFSC</label>
              <input type="text" name="bank_ifsc" class="br-form-control form-control" value="<?= htmlspecialchars($bankIfsc) ?>" placeholder="e.g. HDFC0001234">
            </div>
            <div class="col-12 col-md-6">
              <label class="br-form-label">UPI ID</label>
              <input type="text" name="upi_id" class="br-form-control form-control" value="<?= htmlspecialchars($upiId) ?>" placeholder="e.g. name@upi">
            </div>
          </div>
          <div class="text-subtle small mt-3">Provide either complete bank details or a UPI ID. You can update this anytime.</div>
        </div>
