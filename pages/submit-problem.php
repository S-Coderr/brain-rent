...
if (!$expertUserId) { header('Location: ' . APP_URL . '/pages/browse.php'); exit; }
...
if (!$expert) { header('Location: ' . APP_URL . '/pages/browse.php'); exit; }
...
<main class="py-4" style="padding-top:80px!important">
  <div class="container">
    <div class="mb-3">
      <a href="<?= APP_URL ?>/pages/expert-profile.php?id=<?= $expertUserId ?>" class="text-muted small text-decoration-none">
        <i class="bi bi-arrow-left me-1"></i>Back to Expert Profile
      </a>
    </div>
    <div class="mb-4">
      <h1 class="br-section-title fs-3 mb-1">Submit Your Problem</h1>
      <p class="text-muted small">Submitting to <strong class="text-warning"><?= htmlspecialchars($expert['full_name']) ?></strong> · <?= htmlspecialchars($expert['headline'] ?? '') ?></p>
    </div>
        ...
        <div class="col-12 col-lg-8">
          ...
          <div id="step1">
            <div class="br-profile-section mb-4">
              <h5 class="fw-semibold mb-3 pb-2" style="border-bottom:1px solid var(--br-border)">Describe Your Problem</h5>
              ...
              <!-- Voice Recorder -->
              <div class="mb-3">
                <label class="br-form-label">🎙️ Voice Recording <span class="text-subtle">(optional — often richer than text)</span></label>
                <div class="br-recorder">
                  ...
                  <div id="rec-preview" style="display:none;margin-top:12px;background:var(--br-dark2);border-radius:8px;padding:10px">
              ...
              <div class="mb-3">
                ...
                <div class="br-dropzone" onclick="document.getElementById('file-input').click()">
                  ...
                  <div class="text-subtle" style="font-size:.72rem;margin-top:4px">PDF, images, spreadsheets · Max 10MB each</div>
                  <input type="file" name="attachments[]" id="file-input" multiple style="display:none" onchange="handleFiles(this)">
                </div>
            ...
            <div class="d-flex justify-content-end">
              <button type="button" class="btn br-btn-gold px-4 py-2" id="btn-next">Continue to Review <i class="bi bi-arrow-right ms-1"></i></button>
            </div>
          </div>
          ...
          <div id="step2" style="display:none">
            <div class="br-profile-section mb-4">
              <h5 class="fw-semibold mb-3 pb-2" style="border-bottom:1px solid var(--br-border)">Review Your Submission</h5>

              <div class="br-alert br-alert-info d-flex gap-2 align-items-start mb-4">
                <i class="bi bi-shield-lock-fill text-violet mt-1"></i>
                <span class="small">Your payment will be held in escrow and only released when you confirm satisfaction with the expert's response.</span>
              </div>

              <!-- Problem Summary -->
              <div class="p-3 mb-3" style="background:var(--br-dark3);border-radius:10px">
                <div class="text-subtle" style="font-size:.72rem;text-transform:uppercase;letter-spacing:1px;margin-bottom:6px">Your Problem</div>
                <div class="fw-medium mb-1" id="review-title">—</div>
                <div class="text-muted small" id="review-excerpt">—</div>
              </div>

              <!-- Expert Summary -->
              <div class="d-flex align-items-center gap-3 p-3 mb-3" style="background:var(--br-dark3);border-radius:10px">
                <div class="br-expert-avatar <?= $avColor ?>" style="width:52px;height:52px;font-size:18px;border-radius:12px">
                  <?= strtoupper(substr($expert['full_name'],0,2)) ?>
                </div>
                <div class="flex-grow-1">
                  <div class="fw-medium"><?= htmlspecialchars($expert['full_name']) ?></div>
                  <div class="text-muted small">⭐ <?= number_format($expert['average_rating'],1) ?> · Responds in ~<?= $expert['max_response_hours'] ?>h</div>
                </div>
              ...
              <div class="p-3 mb-4" style="background:var(--br-dark3);border-radius:10px">
                <div class="d-flex justify-content-between small text-muted mb-2"><span>Session fee</span><span class="mono">$<?= number_format($baseFee,2) ?></span></div>
                <div class="d-flex justify-content-between small text-muted mb-2"><span>Urgency add-on</span><span class="mono sidebar-urgency">$0.00</span></div>
                <div class="d-flex justify-content-between small text-muted mb-2"><span>Platform fee (15%)</span><span class="mono">$<?= number_format($platformFee,2) ?></span></div>
                <hr class="br-divider my-2">
                <div class="d-flex justify-content-between fw-semibold"><span>Total charged today</span><span class="mono text-gold sidebar-total">$<?= number_format($total,2) ?></span></div>
              </div>
              ...
              <div class="p-3 mb-3" style="background:var(--br-dark3);border-radius:10px">
                <div class="row g-3">
                  <div class="col-12">
                    <label class="br-form-label">Card Number</label>
                    <input type="text" class="br-form-control form-control mono" placeholder="4242 4242 4242 4242">
                  </div>
                  <div class="col-6">
                    <label class="br-form-label">Expiry</label>
                    <input type="text" class="br-form-control form-control" placeholder="MM / YY">
                  </div>
                  <div class="col-6">
                    <label class="br-form-label">CVV</label>
                    <input type="text" class="br-form-control form-control" placeholder="•••">
                  </div>
                </div>
                <small class="text-subtle d-block mt-2"><i class="bi bi-lock-fill me-1"></i>Secured by Razorpay · SSL encrypted</small>
              </div>
            </div>

            <div class="d-flex justify-content-between">
              <button type="button" class="btn br-btn-ghost px-4" id="btn-back"><i class="bi bi-arrow-left me-1"></i>Back</button>
              <button type="button" class="btn br-btn-gold px-4 py-2" id="btn-pay">
                <i class="bi bi-lock-fill me-2"></i>Pay <span class="sidebar-total">$<?= number_format($total,2) ?></span> & Submit
              </button>
            </div>
          </div>
          ...
          <div id="step3" style="display:none">
            <div class="br-profile-section text-center py-5">
              ...
              <p class="text-muted mb-4"><?= htmlspecialchars($expert['full_name']) ?> has been notified. You'll receive their voice + written response within <?= $expert['max_response_hours'] ?> hours.</p>
              <div class="br-alert br-alert-success d-flex gap-2 align-items-center justify-content-center mb-4" style="max-width:420px;margin:0 auto">
                <i class="bi bi-check-circle-fill text-success"></i>
                <span class="small">Payment held in escrow · You'll be notified by email</span>
              </div>
              <a href="<?= APP_URL ?>/pages/dashboard-client.php" class="btn br-btn-gold px-4 py-2">View in Dashboard <i class="bi bi-arrow-right ms-1"></i></a>
            </div>
          </div>
        ...
        <div class="col-12 col-lg-4">
          ...
          <div class="br-profile-section mb-3">
            ...
            <div class="d-flex flex-column gap-2 small">
              <div class="d-flex justify-content-between text-muted"><span>Session fee</span><span class="mono">$<?= number_format($baseFee,0) ?></span></div>
              <div class="d-flex justify-content-between text-muted"><span>Urgency</span><span class="mono sidebar-urgency">$0</span></div>
              <div class="d-flex justify-content-between text-muted"><span>Platform fee (15%)</span><span class="mono">$<?= number_format($platformFee,0) ?></span></div>
              <hr class="br-divider my-1">
              <div class="d-flex justify-content-between fw-semibold"><span>Total</span><span class="mono text-gold sidebar-total">$<?= number_format($total,0) ?></span></div>
            </div>
          </div>
          ...
          <div class="br-profile-section">
            ...
            <div class="d-flex flex-column gap-2 small text-muted">
              <div class="d-flex gap-2"><span class="text-gold">1.</span><span>Include relevant context — links, data, constraints</span></div>
              <div class="d-flex gap-2"><span class="text-gold">2.</span><span>State what "good" looks like for you</span></div>
              <div class="d-flex gap-2"><span class="text-gold">3.</span><span>Voice recordings often get richer responses</span></div>
              <div class="d-flex gap-2"><span class="text-gold">4.</span><span>One focused problem beats multiple tangled questions</span></div>
            </div>
          </div>
        </div>
