...
$expert = $db->fetchOne(
  "SELECT ep.*, u.full_name, u.profile_photo, u.country
    ...
    <div class="row g-4">
      ...
      <div class="col-12 col-lg-8">
        ...
        <div class="br-profile-header d-flex gap-4 flex-wrap mb-4">
          <div class="br-avatar-lg <?= $avColor ?>" style="flex-shrink:0">
            <?php if ($expert['profile_photo']): ?>
              <img src="<?= htmlspecialchars($expert['profile_photo']) ?>" style="width:100%;height:100%;object-fit:cover;border-radius:20px">
            <?php else: ?>
          ...
          <div class="flex-grow-1">
            <div class="d-flex align-items-center gap-2 flex-wrap mb-1">
              <h1 class="fs-3 fw-bold mb-0" style="font-family:'Playfair Display',serif"><?= htmlspecialchars($expert['full_name']) ?></h1>
            ...
            <div class="d-flex flex-wrap gap-3 small text-muted mb-3">
              <span>⭐ <strong class="text-white"><?= number_format($expert['average_rating'], 1) ?></strong> (<?= $expert['total_reviews'] ?> reviews)</span>
              <span>📋 <strong class="text-white"><?= number_format($expert['total_sessions']) ?></strong> sessions</span>
              <span>⏱️ Responds in <strong class="text-warning">~<?= $expert['max_response_hours'] ?> hours</strong></span>
              <?php if ($expert['country']): ?><span>🌍 <?= htmlspecialchars($expert['country']) ?></span><?php endif; ?>
            </div>
            <div class="d-flex flex-wrap gap-2">
              <?php foreach ($tags as $t): ?>
                <span class="badge" style="background:var(--br-dark3);color:var(--br-text2);border:1px solid var(--br-border);font-weight:400"><?= htmlspecialchars($t) ?></span>
              <?php endforeach; ?>
            </div>
          </div>
        </div>
        ...
        <div class="br-profile-section mb-4">
          ...
          <p class="text-muted small lh-lg"><?= nl2br(htmlspecialchars($expert['bio'] ?? 'This expert has not added a bio yet.')) ?></p>
          <?php if ($expert['current_role'] || $expert['company'] || $expert['linkedin_url']): ?>
            ...
            <div class="row g-3 small">
              ...
              <?php if ($expert['linkedin_url']): ?>
                <div class="col-12"><a href="<?= htmlspecialchars($expert['linkedin_url']) ?>" target="_blank" class="text-violet text-decoration-none small"><i class="bi bi-linkedin me-1"></i>LinkedIn Profile</a></div>
              <?php endif; ?>
            </div>
          <?php endif; ?>
        </div>
        ...
        <div class="br-profile-section">
          <h5 class="fw-semibold mb-3 pb-2" style="border-bottom:1px solid var(--br-border)">
            Client Reviews <span class="text-subtle fw-normal" style="font-size:.85rem">(<?= $expert['total_reviews'] ?> total)</span>
          </h5>
              ...
              <div class="py-3" style="border-bottom:1px solid var(--br-border)">
                <div class="d-flex justify-content-between align-items-start mb-2">
                  <div>
                    ...
                    <div><?php for ($s = 1; $s <= 5; $s++) echo $s <= $r['rating'] ? '⭐' : '☆'; ?></div>
                  </div>
      ...
      <div class="col-12 col-lg-4">
        <div class="br-booking-card">
          <div class="br-booking-price">$<?= number_format($expert['rate_per_session'], 0) ?> <span class="text-muted" style="font-size:1rem;font-family:'DM Sans',sans-serif;font-weight:400">/ session</span></div>
          ...
          <div class="d-flex flex-column gap-2 mb-4 small text-muted">
            ...
            <div><i class="bi bi-check-lg text-success me-2"></i>Response within <?= $expert['max_response_hours'] ?> hours</div>
            <div><i class="bi bi-check-lg text-success me-2"></i>Escrow-protected payment</div>
          </div>
          <a href="<?= APP_URL ?>/pages/submit-problem.php?expert_id=<?= $expertUserId ?>" class="btn br-btn-gold w-100 py-3 mb-2 fw-semibold">
